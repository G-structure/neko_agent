...
# ---
# WebSocket Signaler and Event Broker
# ---

class Broker:
    """Fan-out minimal event bus keyed by high-level topics.

    Messages are routed to queues by broad event prefixes so consumers can
    await only the topics they care about.

    :ivar queues: Mapping of topic name to asyncio.Queue used for fan-out.
    ...
    def topic_queue(self, topic: str, maxsize: int = 512) -> asyncio.Queue:
        """Return (and lazily create) the queue for a topic.

        :param topic: The topic name (e.g., "signal", "system", "misc").
        :type topic: str
        :param maxsize: Max queued messages to buffer for this topic.
        :type maxsize: int
        :return: The queue for the topic.
        :rtype: asyncio.Queue
        """
    ...
    def publish(self, msg: Dict[str, Any]) -> None:
        """Publish a message to an appropriate topic based on its event name.

        :param msg: The JSON-decoded message from the server.
...
class Signaler:
    """WebSocket client with background read/write tasks.

    Manages a WebSocket connection to the server, providing:
    - A send queue so callers can enqueue JSON messages safely.
    - Background read loop that publishes incoming events to a `Broker`.
    - An `_closed` event that is set when connection loops stop.

    :ivar url: The WebSocket URL (including token).
    :vartype url: str
    :ivar wsopts: Options passed to `websockets.connect`.
    :vartype wsopts: Dict[str, Any]
    :ivar ws: The active WebSocket (or None before connected).
    :vartype ws: Optional[websockets.WebSocketClientProtocol]
    :ivar _tasks: Set of background asyncio tasks for this connection.
    :vartype _tasks: Set[asyncio.Task]
    :ivar _sendq: Outbound message queue (dicts that will be JSON-encoded).
    :vartype _sendq: asyncio.Queue
    :ivar broker: Fan-out broker for routing incoming events.
    :vartype broker: Broker
    :ivar _closed: Event set when either read or send loop ends.
    ...
    async def connect_with_backoff(self) -> "Signaler":
        """Connect with exponential backoff and start background loops.

        :return: Self (connected and with loops running).
        :rtype: Signaler
        """
        backoff = 1
        while not self._closed.is_set():
            try:
                ...
                logger.error("WS connect error: %s - retrying in %ss", e, backoff)
    ...
    async def _read_loop(self) -> None:
        try:
            ...
            async for raw in self.ws:
                logger.debug("Received message: %s", raw[:100] if len(raw) > 100 else raw)
                try:
                    msg = json.loads(raw)
        ...
        try:
            while not self._closed.is_set():
                ...
                try:
                    ...
                    logger.warning("Send failed: connection is closed. Re-queuing message and exiting send loop.")
    ...
    async def send(self, msg: Dict[str, Any]) -> None:
        """Queue a JSON-serializable message to be sent.

        :param msg: The message dict to send.
        :type msg: Dict[str, Any]
        """
        await self._sendq.put(msg)
